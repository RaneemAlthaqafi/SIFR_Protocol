...
Figure~\ref{fig:adversary} visualizes the same data on a log-scale axis, color-coded by rejecting error class. The slowest path (replay, 2.4 ms) subsumes one full authorize+execute round; the fastest (WASM-without-grant, 2.2 us) returns before any transport, signature, or runtime work happens.
